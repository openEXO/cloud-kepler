...
def rmscluster(dtime, dflux, segstart, segend, out, interestfactor = 1.0):
    '''
    Adds another clustering method for the algorithm to supplement the DBscan method
    currently implemented in clean_signal. First it calculates the RMS of the data
    in each detrended segment, then uses a k-means approach to find two distinctly 
    different RMS values that segments cluster around.

    :param dtime: The full time array of the detrended data for the algorithm to run on
    :type dtime: np.ndarray

    :param dflux: The full detrended flux array for the algorithm to run on
    :type dflux: np.ndarray

    :param segstart: The array of start-times for each segment of the detrended data
    :type segstart: np.ndarray

    :param segend: The array of end-times for each segment of the detrended data
    :type segend: np.ndarray

    :param out: Output from the BLS algorithm
    :type out: dict

    :param interestfactor: Threshold of signficance for the centroids output from the k-means to be considered distinct
    :type interestfactor: float
    '''

    splitrms = []

    #Only look at segments with valid dip/blip measurements
    ndx = np.where((out['srsq_dip'] > 0.) & (out['srsq_blip'] > 0.))

    #For each such valid segment, measure the rms of finite detrended flux values for that segment
    for i in range(len(segend[ndx])):
        segmask = (dtime >= segstart[ndx][i]) & (dtime< segend[ndx][i]) & np.isfinite(dflux)
        rms = np.sqrt(np.mean(np.square(abs(dflux[segmask]))))
        splitrms = np.append(splitrms, rms)
    
    #Find two RMS values that all the segments cluster around
    centers, clusters = kmeans(splitrms, 2)
    maxcenter = np.argmax(centers)
    mincenter = np.argmin(centers)
    
    #If higher value centroid is sufficiently separate from first,
    #Find the indices of all segments that belong to the cluster
    #around the higher value centroid. Presumably, these belong to
    #a single source of 'error' i.e., a transit signal
    if centers[maxcenter] - centers[mincenter] >= interestfactor*np.std(splitrms):
        indices = np.where(clusters == maxcenter)[0]
        indices = indices.flatten()
        
    else:
        print 'No interesting clusters. Check for single outliers next'
        indices = []
    return indices
...
def main():
    ...
    for k, q, time, flux, fluxerr in read_mapper_output(sys.stdin):
        ...
        for i in xrange(cfg['clean_max']):
            ...
            #Run the RMScluster method to find possible signals that DBscan may miss
            indices = rmscluster(dtime, dflux, segstart, segend, bls_out)

            try:
                clean_out = clean_signal(time, flux, dtime, dflux, dfluxerr,
                    bls_out, indices, model=cfg['model'])
            except RuntimeError:
                break
