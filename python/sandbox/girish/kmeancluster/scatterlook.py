...
def main(fname, datasrc, datapath = None, position = 'first'):
	...
	if position == 'first':
		...
		print 'Invalid position given. Either use \'first\' or \'final\''
	...
	for i in range(len(segend)):
		...
		splitsegstart = np.append(splittime, segstart[i])
		splitsegend = np.append(splittime, segend[i])
	...
	#Create clusters by rms value

	centers, clusters = kmeans(splitrms, 2)
	maxcenter = np.argmax(centers)
	mincenter = np.argmin(centers)
	#If higher centroid is sufficiently separate from first, create new subset
	interestfactor = 1.0
	if centers[maxcenter] - centers[mincenter] >= interestfactor*np.std(splitrms):
		indices = np.where(clusters == maxcenter)
		print '%d out of %d show higher RMS-values' % (len(indices), len(splitrms))
		print centers
