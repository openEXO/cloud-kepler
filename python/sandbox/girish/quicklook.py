...
def __onclick(event, ax, kdtree, segstart, segend, duration_dip, depth_dip,
    ...
    #Trend residuals
    normfactor = np.median(flux[mask1])
    trendtime = []
    resid2 = []

    for i in range(len(flux[mask1])):
        for j in range(len(dflux[mask])):
            if time[mask1][i] == dtime[mask][j]:
                trendtime = np.append(trendtime, dtime[mask][j])
                
                residual = flux[mask1][i] - (flux[mask1][i]/(dflux[mask][j]+1.))
                residual = residual/normfactor

                resid2 = np.append(resid2,residual)
                
    
    
    
    ax3.scatter(trendtime, resid2, label = 'Residuals')
    ...
    ptp2 = np.amax(resid2) - np.amin(resid2)
    ax3.set_xlim(segstart[which], segend[which])
    ax3.set_ylim(np.amin(resid2) - 0.1 * ptp2, np.amax(resid2) + 0.1 * ptp2)


    ax3.set_xlabel('Time (days)')
    ax1.set_ylabel('Flux')
    ax2.set_ylabel('Flux')
    ax3.set_ylabel('Fraction of Raw Flux')
    plt.legend(loc = 'best')
    plt.tight_layout()
    plt.show()
